...
The Objective: Destroy the enemy by achieving one of the victory conditions (listed in Section 8). 
...
This is the game's most critical mechanic. Your units rely on a "network" to function. 
     ...
     Going Offline: If a unit is not connected, it goes "Offline."
         ...
         Relays: Unlike other units, Relays are always treated as Online, and can always move.  However, they can only bring other units online if they are connected to the communication network.
         OPTIONAL RULE: In the rare case that a line of communication activates a combat unit, and that combat unit is adjacent to a relay which is NOT in the LOC, pykrieg rules that the relay is activated by the adjacency LOC rule, and therefore propogates its own lines of communication.  However, in the original rules by Debord it is unclear if this is the case, so this special case can be toggled off by the user.
      
Blocking the Network:
Lines of Communication are blocked by Mountains and Enemy Units. 

     Note: Lines are not blocked by Mountain Passes.
...
5. Setup 
    ...
    Each player places their units freely within their own territory. 
    Opponent set-ups are hidden until the game begins. 
...
A. Movement Phase 
     ...
     Mounted Units (Cavalry, Swift Cannon, Swift Relay): If they move out of their lines of communication, they stop on the first square they enter in which they go offline.     
...
B. Combat Phase 
     ...
     Line of Sight: Attacks and defensive support can pass through friendly units, but are blocked by enemy units.
...
Combat is calculated by comparing power numbers along straight lines (vertical, horizontal, or diagonal). 
    ...
    Calculate Attack Power: Sum the Attack stats of all your units in a direct line with the target enemy unit. 
    Calculate Defense Power: Sum the Defense stats of all enemy units in a direct line supporting the target. 
    Compare the Totals:
         Offense ≤ Defense: The attack fails. The unit is secure.
         Offense = Defense + 1: Forced Retreat. The defender must move that unit at the start of their next turn. If there is no valid square to retreat to, the unit is captured (destroyed).
...
To win the game, you must completely destroy the enemy's capacity to fight. You win instantly if you accomplish one of the following: 
    ...
    Total Annihilation: You destroy the last of your opponent's units. 
    Network Collapse: None of your opponent's units are online, or you destroy both of their arsenals.
    Surrender: Your opponent declares you the victor. 